...
export default function Financials() {
  ...
  // Fallback: estimate from invoices/bills if no accounts data
  // Use outstanding receivables as current assets proxy (not netIncome twice)
  ...
  const totalAssets = accountAssets || (outstandingReceivables + Math.max(netIncome, 0) * 0.5);
  const totalLiabilities = accountLiabilities || outstandingPayables;
  const equity = accountEquity || (totalAssets - totalLiabilities);
