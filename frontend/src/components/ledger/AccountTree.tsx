...
export function AccountTree({ accounts, selectedAccountId, onSelectAccount, transactionCounts }: AccountTreeProps) {
    ...
    <div className="space-y-3">
      ...
      <Accordion type="multiple" defaultValue={filteredTypes} className="space-y-1">
        {filteredTypes.map((type) => {
          ...
          const typeCount = accts.reduce((s, a) => s + (transactionCounts.get(a.account_name) || 0), 0);
            ...
            <AccordionItem key={type} value={type} className="border-none">
              <AccordionTrigger className="hover:no-underline py-2 px-2 text-sm">
                <div className="flex items-center gap-2 w-full">
                  ...
                  {typeCount > 0 && (
                    <span className="text-xs text-muted-foreground ml-auto mr-2">{typeCount}</span>
                  )}
                </div>
              </AccordionTrigger>
              <AccordionContent className="pb-1">
                <div className="space-y-0.5 pl-4">
                  {filtered.map((acct) => {
                    const count = transactionCounts.get(acct.account_name) || 0;
                    return (
                      <button
                        key={acct.id}
                        className={`w-full text-left text-xs px-3 py-1.5 rounded transition-colors flex items-center justify-between ${
