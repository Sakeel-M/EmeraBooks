...
export function InvoicePreview({ invoice, companyProfile, templateSettings, items = [], compact = false }: InvoicePreviewProps) {
  ...
  const taxTotal = showTax ? items.reduce((s, i) => s + (i.quantity * i.unit_price * (i.tax_rate || 0) / 100), 0) : 0;
  ...
  if (template === "modern") {
    return (
      <div className={`bg-white text-black ${compact ? "text-xs p-4" : "p-8"} rounded-lg`}>
        ...
        <InvoiceBody invoice={invoice} companyProfile={companyProfile} items={items} subtotal={subtotal} taxTotal={taxTotal} total={total} showTax={showTax} showTerms={showTerms} showNotes={showNotes} footerText={footerText} compact={compact} />
      </div>
    );
  }
      ...
      <div className={`bg-white text-black ${compact ? "text-xs p-4" : "p-8"} rounded-lg`}>
        ...
        <div className="flex justify-between mb-8">
          ...
          <div className="text-right">
            <p className="text-sm text-gray-500">Issued: {invoice?.invoice_date ? new Date(invoice.invoice_date).toLocaleDateString() : ""}</p>
            <p className="text-sm text-gray-500">Due: {invoice?.due_date ? new Date(invoice.due_date).toLocaleDateString() : ""}</p>
          </div>
        </div>
        <InvoiceBody invoice={invoice} companyProfile={companyProfile} items={items} subtotal={subtotal} taxTotal={taxTotal} total={total} showTax={showTax} showTerms={showTerms} showNotes={showNotes} footerText={footerText} compact={compact} />
      </div>
    ...
    <div className={`bg-white text-black ${compact ? "text-xs p-4" : "p-8"} rounded-lg`}>
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="text-right">
          ...
          <p className="text-sm text-gray-500 mt-1">Date: {invoice?.invoice_date ? new Date(invoice.invoice_date).toLocaleDateString() : ""}</p>
          <p className="text-sm text-gray-500">Due: {invoice?.due_date ? new Date(invoice.due_date).toLocaleDateString() : ""}</p>
        </div>
      </div>
      <InvoiceBody invoice={invoice} companyProfile={companyProfile} items={items} subtotal={subtotal} taxTotal={taxTotal} total={total} showTax={showTax} showTerms={showTerms} showNotes={showNotes} footerText={footerText} compact={compact} />
...
function InvoiceBody({ invoice, items, subtotal, taxTotal, total, showTax, showTerms, showNotes, footerText, compact }: any) {
  return (
    <>
      {/* Bill To */}
      <div className="mb-6">
        ...
        <tbody>
          {items.length > 0 ? items.map((item: any, i: number) => (
            <tr key={i} className="border-b border-gray-100">
              ...
              <td className="text-right py-2">${(item.quantity * item.unit_price * (1 + (item.tax_rate || 0) / 100)).toFixed(2)}</td>
            </tr>
          )) : (
      ...
      <div className="flex justify-end mb-6">
        <div className={compact ? "w-40" : "w-56"}>
          <div className="flex justify-between py-1"><span className="text-gray-500">Subtotal</span><span>${subtotal.toFixed(2)}</span></div>
          {showTax && <div className="flex justify-between py-1"><span className="text-gray-500">Tax</span><span>${taxTotal.toFixed(2)}</span></div>}
          <Separator className="my-1" />
          <div className="flex justify-between py-1 font-bold text-lg"><span>Total</span><span>${total.toFixed(2)}</span></div>
        </div>
      </div>
