...
export function InvoiceForm({
  ...
  useEffect(() => {
    if (open) {
      fetchCustomers();
      if (invoice) {
        // Fetch line items and reset form for editing
        supabase
          .from("invoice_items")
          .select("*")
          .eq("invoice_id", invoice.id)
          .then(({ data: existingItems }) => {
            const items = existingItems && existingItems.length > 0
            ...
            form.reset({
              customer_id: invoice.customer_id || "",
              invoice_number: invoice.invoice_number || "",
              invoice_date: invoice.invoice_date ? parseLocalDate(invoice.invoice_date) : new Date(),
              due_date: invoice.due_date ? parseLocalDate(invoice.due_date) : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
              currency: invoice.currency || "USD",
              terms: invoice.terms || "",
              notes: invoice.notes || "",
              status: invoice.status || "draft",
