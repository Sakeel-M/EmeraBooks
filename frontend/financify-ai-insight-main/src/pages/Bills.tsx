...
import { Plus, Upload, FileText, ChevronRight, Tags, X } from "lucide-react";
...
export default function Bills() {
  ...
  // Resolve category: db value → guessed from vendor name → guessed from description → raw bank category map → "Other"
  const getBillCategory = (bill: any): string =>
    bill.category || guessCategory(bill.vendors?.name) || guessCategory(bill.description) || mapRawBankCategory(bill.description) || "Other";

  // Unique auto-mapped sectors from bills (for CategoryManager filtering)
  const billSectors = useMemo(() => {
    const cats = new Set<string>();
    (bills || []).forEach((b) => {
      const cat = getBillCategory(b);
  ...
  const filteredBills = useMemo(() => {
    let result = (bills || []).filter((b) => activePipeline?.statuses.includes(b.status || "draft"));
    if (selectedCategory) {
      result = result.filter((b) => getBillCategory(b) === selectedCategory);
    }
    return result;
  }, [bills, activePipeline, selectedCategory]);

  const columns: ColumnDef<any>[] = [
    { accessorKey: "bill_number", header: "Bill Number", cell: ({ row }) => (
    ...
    { accessorKey: "category", header: "Category", cell: ({ row }) => {
      const cat = getBillCategory(row.original);
  ...
  return (
    <Layout>
      <div className="space-y-6">
        ...
        <BillDetailSheet open={detailOpen} onOpenChange={setDetailOpen} bill={detailBill} onEdit={handleEdit} onDelete={handleDelete} onRefresh={refetch} />
